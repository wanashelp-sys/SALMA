...
export default function CountingGame() {
  ...
  const generateQuestion = () => {
    const correct = getRandomNumber(3, 12)
    const opts = [correct]
  ...
  const handleAnswer = (selected: number) => {
    ...
    const correct = selected === correctCount
  ...
  return (
    <div>
      ...
      <div className="text-center mb-6">
        <p className="text-lg font-medium text-salma-text">كم عدد البالونات؟ 🎈</p>
      </div>

      <div className="bg-gradient-to-br from-salma-pink-light to-salma-orange-light rounded-2xl p-12 mb-8 text-center">
        <div className="text-5xl leading-relaxed" style={{ letterSpacing: '0.5rem' }}>
          {'🎈'.repeat(correctCount)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {options.map((num) => {
          ...
          const isCorrectAnswer = num === correctCount
